...
void BackEmfMotor::Initialize(int pin1, int pin2, int analogPin) {
	fHBridge.Initialize(pin1, pin2);
//#else
...
void BackEmfMotor::UpdatePwm() {
	...
	if (fPwmMicros < -kMaxPwmMicros)
	  fPwmMicros = -kMaxPwmMicros;
	else if (fPwmMicros > kMaxPwmMicros)
	  fPwmMicros = kMaxPwmMicros;
